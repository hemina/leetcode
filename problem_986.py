...
class Solution:
    def intervalIntersection(self, A: List[List[int]], B: List[List[int]]) -> List[List[int]]:
        ...
        while ind_a < len(A) and ind_b < len(B):
            interval_a = A[ind_a]
            interval_b = B[ind_b]
            head = max(interval_a[0], interval_b[0])
            tail = min(interval_a[1], interval_b[1])
            if tail >= head:
...
"""
Runtime: 148 ms, faster than 96.27% of Python online submissions.
Memory Usage: 14.3 MB, less than 6.06% of Python online submissions.
Complexity: O(m+n)
"""
