...
The groups are separated by colons (":"). For example,
the address 2001:0db8:85a3:0000:0000:8a2e:0370:7334 is a valid one.
...
class Solution:
    def validIPAddress(self, IP: str) -> str:
        v4 = re.fullmatch("([0-9]+)\.([0-9]+)\.([0-9]+)\.([0-9]+)", IP)
        if v4:
            for num in v4.groups():
                if str(int(num)) != num or int(num) > 255:
                    return "Neither"
            return "IPv4"
        v6 = re.fullmatch(
            "([0-9a-fA-F]{1,4}|0):([0-9a-fA-F]{1,4}|0):([0-9a-fA-F]{1,4}|0):([0-9a-fA-F]{1,4}|0):"
            "([0-9a-fA-F]{1,4}|0):([0-9a-fA-F]{1,4}|0):([0-9a-fA-F]{1,4}|0):([0-9a-fA-F]{1,4}|0)",
            IP)
        if v6:
            return "IPv6"
...
"""
Runtime: 28 ms, faster than 69.38% of Python online submissions.
Memory Usage: 14 MB, less than 28.56% of Python online submissions.
Complexity: O(n)
"""
